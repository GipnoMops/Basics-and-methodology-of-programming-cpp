...
size_t CountValues(const char* data, size_t size) {
    ...
    while (byte_number < size) {
        try {
            elem = *(reinterpret_cast<const size_t*>(data));
            ++value;
...
int main() {
    ...
    size_t num = 3;
    const size_t* ptr_num = &num;
    std::cout << ptr_num << " " << *ptr_num << "\n";

    const char* num_char_ptr = reinterpret_cast<const char*>(num);
    std::cout << &num_char_ptr << "\n";

    //std::cout << reinterpret_cast<const size_t*>(num_char_ptr) << "\n";
