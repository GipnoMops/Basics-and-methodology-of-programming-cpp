...
class BiMap {
private:
    std::map<std::optional<Key1>, std::shared_ptr<Value>> map1_;
    std::map<std::optional<Key2>, std::shared_ptr<Value>> map2_;
...
public:
    ...
    void Insert(const std::optional<Key1>& key1, const std::optional<Key2>& key2, const Value& value) {
        if (key1 && key2) {
            if (map1_.find(key1) == map1_.end() && map2_.find(key2) == map2_.end()) {
