...
public:
    ...
    Polynomial<T> operator/ (const Polynomial<T>& other) const {
        ...
        for (int i = 0; i <= ansver.Degree(); ++i) {
            ansver.odds_[ansver.Degree() - i] = temp[temp.Degree() - i] / other[other.Degree()];
            for (int j = 0; j <= other.Degree(); ++j) {
                temp.odds_[temp.Degree() - j - i] -= ansver[ansver.Degree() - i] *
                        other[other.Degree() - j];
            }
        }
        return ansver;
    }
...
int main() {
    std::vector<int> vec1 = {2, 2, 4, 4};
    std::vector<int> vec2 = {2, 2};
    std::vector<int> vec3 = {1, 2, 3};
    Polynomial<int> p1(vec1);
    Polynomial<int> p2(vec2);
    Polynomial<int> p3(vec3);
    Polynomial<int> p4(4);
